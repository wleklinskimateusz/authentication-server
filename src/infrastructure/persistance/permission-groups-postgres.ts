...
const permissionGroupRowSchema = z
  .object({
    id: z.string(),
    name: z.string(),
...
export class PermissionGroupPostgres implements PermissionGroupRepository {
  ...
  private async parseRow(row: object): Promise<PermissionGroup> {
    const { id, ...rest } = this.rowSchema.parse(row);
    return new PermissionGroup({
      id,
      ...rest,
      permissions: await this.permissionRepository.findGroupPermissions(id),
    });
  }

  async createGroup(group: PermissionGroup, userId: string): Promise<void> {
    await sql`
  ...
  async updateGroup({ name, id, description }: PermissionGroup): Promise<void> {
    await sql`
      UPDATE groups 
  ...
  async addPermissionsToGroup(
    ...
    await sql`
      INSERT INTO group_permissions ${sql(
  ...
  async findUserGroups(userId: string): Promise<PermissionGroup[]> {
    const result = await sql<PermissionGroup[]>`
      SELECT id, name, description, created_at, updated_at 
      FROM groups g
  ...
  async searchGroups(
    ...
    const andClause = fields.length
      ? sql`AND ${sql(
          fields
            .map(([key, value]) => sql`pg.${sql(key)} ILIKE ${`%${value}%`}`)
