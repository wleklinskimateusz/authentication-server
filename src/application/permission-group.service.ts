...
export class PermissionGroupService {
  ...
  async updateGroup(
    ...
    if (!group) {
      throw new NotFound(`cannot update group with id ${id}`);
    }
  ...
  async getUserGroups(userId: string): Promise<PermissionGroup[]> {
    const groups = await this.permissionGroupRepository.findUserGroups(userId);
  ...
  async addPermissionsToGroup(
    ...
    const group = await this.permissionGroupRepository.findGroupById(groupId);
    if (!group) {
      throw new NotFound(`cannot add permission to group with id ${groupId}`);
    }
  ...
  async deleteGroup(id: string): Promise<void> {
    const group = await this.permissionGroupRepository.findGroupById(id);
    if (!group) {
      throw new NotFound(`cannot delete group with id ${id}`);
    }
  ...
  async searchGroups(
    ...
    const fields = Object.entries(filters).filter(([, v]) => v !== undefined);
    try {
      if (fields.length === 0) {
        return await this.getUserGroups(userId);
      }
      return await this.permissionGroupRepository.searchGroups(filters, userId);
    } catch (error) {
  ...
  async getGroupsByName(name: string): Promise<PermissionGroup[]> {
    const group = await this.permissionGroupRepository.findGroupsByName(name);
    if (!group) {
      throw new NotFound(`Group with name ${name} not found`);
    }
    return group;
  }
}
